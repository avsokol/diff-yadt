	   ...
	   char **output_placement)
{
  ...
  *ap = 0;
  ...
  /* 
     Seems quote_system_arg mechanism has bugs when diff3 is executed
     from the path with spaces, or file pathnames contain spaces
     we quote here diff_program and every file with double-quotes.
     Also, we quote the whole command with doube-quotes.
     Tests show it works.
  */

  FILE *fpipe;
  char const args[] = " --horizon-lines=100 --";
  char *command = xmalloc (strlen(diff_program)
			   + sizeof " -a"
			   + sizeof " -w"
			   ...
			   + strlen(fileb) + 1
			   + 8);
  /* 8 - number of quotes we add during creating command */

  strcpy(command, "\"");

  strcat(command, "\"");
  strcat(command, diff_program);
  strcat(command, "\"");
  ...
  if (strip_trailing_cr) {
    strcat (command, " --strip-trailing-cr");
  }
  strcat (command, args);
