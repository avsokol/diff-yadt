...
#  $Id: entry.tcl,v 1.2 2008/12/09 13:37:01 shlyua Exp $
# ------------------------------------------------------------------------------
#  Index of commands:
#     - Entry::create
#     - Entry::configure
...
proc Entry::configure { path args } {
    ...
    set vars [list chstate cheditable chfg chdfg chbg chdbg chtext chcursor]
    set opts [list -state -editable -foreground -disabledforeground \
                -background -disabledbackground -text -cursor]
    ...
    if { $cheditable || $chcursor } {
        set cursor   [$path cget -cursor]
        if { $cursor == "" } {
            if [Widget::getMegawidgetOption $path -editable] {
                set cursor "xterm"
            } else {
                set cursor "left_ptr"
            }
        }
        $path:cmd configure -cursor $cursor
    }
