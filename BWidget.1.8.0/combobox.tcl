...
#  $Id: combobox.tcl,v 1.2 2008/12/09 13:32:20 shlyua Exp $
# ----------------------------------------------------------------------------
#  Index of commands:
#     - ComboBox::create
#     - ComboBox::configure
...
#     - ComboBox::_maplist_and_validate_input
#     - ComboBox::_validate_listbox
#
# ----------------------------------------------------------------------------
...
proc ComboBox::create { path args } {
	...
	if [Widget::getMegawidgetOption $path -autocomplete] {
	    ::bind $entry <KeyRelease> "ComboBox::_maplist_and_validate_input $path %A %k"
	}
    } else {
...
proc ComboBox::_maplist_and_validate_input { path key key_code } {
    ...
    ComboBox::_validate_listbox $path $key $key_code
}
...
proc ComboBox::_create_popup { path } {
    ...
    ::bind $listb <KeyRelease>      "ComboBox::_validate_listbox $path %A %k"
...
proc ComboBox::_validate_listbox { path key key_code  } {

    set listb $path.shell.listb
    set old_value [Entry::cget $path.e -text]

    if { $key != "" } {
        set new_value "$old_value$key"
        set id [ $listb curselection ]
        if { $id == "" } { set id 0 }        
    } else {
        # process 'Delete' key only:
        if { $key_code != 8 } return
        set new_value [ string replace $old_value end end ]
        set id 0
    }

    foreach val [ lrange [Widget::cget $path -values] $id end ] {
        if [ string match -nocase $new_value* $val ] {
            Entry::configure $path.e \
                    -text [ string range $val 0 [ expr [ string length $new_value ] - 1 ] ]
            $listb selection clear 0 end
            $listb selection set $id
            $listb activate $id
            $listb yview $id
            return
        }
...
proc ComboBox::_mapliste { path } {
    ...
    if { [set idx [lsearch -exact $values $curval]] != -1 ||
         [set idx [lsearch -glob $values "$curval*"]] != -1 } {
        if {$bw} {
